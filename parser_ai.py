...
4. Поле "подкатегория" определяется ТОЛЬКО по структуре сообщения, а не по типу устройства.
   Никогда не выдумывай подкатегорию на основе типа техники (например, "смартфон", "камера", "дрон").
   Если в сообщении нет явных заголовков подкатегорий — укажи "Без типа".
...
7. Определи подкатегорию по структуре сообщения:
   - Если в сообщении встречаются строки, которые выглядят как заголовки подкатегорий 
     (например: "Poco", "Redmi", "Xiaomi Pad", "Apple", "Samsung", "GoPro", "HyperX", "Oura Rings", "Logitech", "Whoop", "Camera" и т.д.), 
     считай их подкатегориями.
   - Такие строки часто:
       • находятся отдельно от товаров,
       • содержат только одно или два слова,
       • не содержат цен, чисел или символов валюты,
       • могут включать эмодзи, ссылки, флаги — их нужно игнорировать.
   - Все товары, идущие после такой строки, принадлежат этой подкатегории,
     пока не встретится новый подобный заголовок.
   - Если строка выглядит как тип устройства ("Camera", "Watch", "Tablet") и стоит в отдельной строке — это тоже подкатегория.

8. Очистка подкатегорий:
   - Удали эмодзи, флаги (🇷🇺, 🇪🇺 и т.п.), ссылки, скобки и любые спецсимволы.
   - Приведи подкатегорию к нормальному виду с большой буквы (например: "GoPro", "Camera", "Logitech", "Oura Rings").
   - Если строка в верхнем регистре — преобразуй её к обычному ("PLAID NOTE" → "Plaid Note").

9. Если подкатегория не найдена в сообщении — укажи "Без типа".
