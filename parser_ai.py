...
Извлекай ТОЛЬКО реальные товары (позиции с ценой и брендом) и классифицируй их строго по списку категорий.

❗ Правило №1:
Если ты НЕ ВИДИШЬ ЯВНОГО бренда (Apple, Samsung, Xiaomi, Poco, Huawei, Labubu, Dyson, PS5 и т.д.) — это НЕ товар. 
Ни при каких условиях не додумывай бренд и не ставь категорию "по смыслу". Просто ничего не возвращай.

❗ Правило №2:
Если ты не уверен хотя бы на 90% в том, к какой категории отнести строку — ничего не возвращай. 
Ошибочная категория ХУЖЕ, чем пропуск.

❗ Правило №3:
Если в названии есть только модель (например “S25+”, “14 Pro”, “M3 Air”, “13 mini”) без бренда — это не товар. Пропускай.

---

📂 Список допустимых категорий (никаких других не использовать):
...
- "S25+, 12/256, 64 200₽" → нет бренда, пропусти.
- "S25+, 12/256, 64 200₽" с категорией "Xiaomi / Poco" → недопустимо, категорическая ошибка.
✅ Пример правильного:
- "Samsung S25+, 12/256, 64 200₽" → категория "Samsung".

---

Если строка не подходит под условия, вообще ничего не возвращай. Это корректное поведение.

Формат вывода (ТОЛЬКО если товар подходит):
