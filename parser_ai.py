...
7. Определи подкатегорию по структуре сообщения:
   - Если в сообщении встречаются строки, которые выглядят как заголовки подкатегорий 
     (например: "Poco", "Redmi", "Xiaomi Pad", "Apple", "Samsung", "GoPro", "HyperX", "Oura Rings", "Logitech" и т.д.), 
     считай их подкатегориями.
   - Такие строки часто:
       • записаны заглавными буквами или начинаются с большой буквы,
       • содержат только одно или два слова (например: "Logitech", "Oura Rings"),
       • не содержат цен, чисел или символов валюты,
       • могут содержать эмодзи, ссылки, флаги — но их нужно игнорировать.
...
8. Очистка подкатегорий:
   - Перед записью убери все эмодзи, ссылки, флаги (🇷🇺, 🇪🇺 и т.п.), скобки и лишние символы.
   - Приведи подкатегорию к строчной форме (например: "hyperx", "oura rings", "logitech").
   - Если строка полностью в верхнем регистре (например, "LOGITECH" или "PLAUD NOTE") — 
     преобразуй её в нормальный вид ("Logitech", "Plaud Note"), а потом пиши с большой буквы ("Logitech", "Plaud note").
