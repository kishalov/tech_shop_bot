...
1. Строка описывает товар, если в ней есть цена и структура похожа на: 
   "Название товара - характеристики - стоимость".
   Названием товара может считаться только строка, где есть указание бренда.

2. Если в строке **нет бренда**, но предыдущие строки содержали товары, 
   начинающиеся с **тех же букв модели** (например, A06, A07, A16, A26), 
   используй тот же бренд, что и в предыдущем товаре. 
   Например: если раньше был “Samsung A06”, то “A16” и “A26” тоже относятся к Samsung.

3. Названия категорий и подкатегорий должны строго соответствовать бренду:
   - Если бренд Samsung → категория = “Samsung”, подкатегория = “Смартфон”.
   - Если бренд Apple → категория = “Apple”, подкатегория = “Iphone”.
   - Если бренд неизвестен → категория = “Неизвестный бренд”, подкатегория = “Неизвестный тип”.

4. Если модель не указала бренд явно, но он однозначно определяется по названию (например, "A06" → Samsung, "Iphone" → Apple),
   всё равно заполни поле "категория" корректным брендом.
"""
