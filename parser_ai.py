...
🧭 Логика:
   ...
   Строка описывает товар, если ее структура похожа на: "Название товара - характеристики товара - стоимость товара". Названием товара может считаться только название с упоминанием названия существующего бренда, а если нет - то это не название товара, пример: "IPhone 17 air" - это название товара, а "+3 months" - это не название товара, даже если в этой строке есть цена, потому что не указан бренд. Иногда могут попадаться строки, в которых нет цен, но есть названия товаров и/или характеристики товаров - такие строки нужно пропускать. Иногда могут встречаться строки с характеристиками товара и ценой, но без названия товара - это могут быть разные вариации одного товара, который был указан выше - нужно добавить эти строки как товары, но обязвтельно добавить в название родительское название товара (то есть навзвание товара, которое было написано сразу перед строками в которых нет названия товара, но есть характеристики и цена). Названия категорий должны основываться на бренде, а названия подкатегории на типе товара, например: если товар называется "IPhone 17 Air", то тут бренд Apple, а тип товара - Iphone, значит категория будет называться "Apple" а подкатегория "Iphone".
"""
