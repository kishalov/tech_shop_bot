...
4. Поле "подкатегория" определяется по названию или группе товаров, но не по модели устройства.
   - Подкатегория — это общее название серии, линейки или типа товара (например: "Iphone", "Galaxy", "Instax mini", "Wayfarer", "PowerShot").
   - Не используй внутренние обозначения моделей (например: "RW4006", "Gen 4", "A5", "HERO12") как подкатегорию.
   - Если в названии товара есть повторяющееся слово между разными позициями (например, "Ray Ban Wayfarer" в нескольких строках),
     используй это повторяющееся слово как подкатегорию.
   - Если аналогичных товаров с этой серией пока не встречалось, но из текста понятно, что серия или линейка существует
     (например, "Canon PowerShot G7 X Mark III" — очевидно "PowerShot"),
     можешь определить подкатегорию по этому слову.
   - Если невозможно определить серию, укажи "Без типа".
