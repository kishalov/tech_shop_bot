...
4. Поле "подкатегория" определяется по типу товара - например если товар назвается "Iphone 15" - то его подкатегория "Iphone" и так далее. Если несколько товаров подходят под одну подкатегорию - не придумывай новую и не уточняй, а ставь ту подкатегрию, которая уже указана у аналогичных товаров. Если подкатегория не найдена даже после анализа товара и аналогичных ему других товаров — укажи "Без типа".
