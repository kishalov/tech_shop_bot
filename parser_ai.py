...
🧭 Логика:
1. Определи, является ли строка товаром: в ней должна быть цена и название и в названии не должно быть рекламных слов вроде "Акция", "Гарантия", "Подписка" и т.п. Если в строке нет явно прописанного имени бренда (например "Apple", "Iphone", "Xiaomi", "Labubu", "PS5" и т.п.) - это не товар, пропускай.
2. Распознай тип (смартфон, ноутбук, игрушка, смарт-часы и т.д.) и выбери подходящую категорию из списка. Если ты не можешь распознать тип товара (только в самых крайних случаях, если в строке нет ни одного осмысленного названия вроде "iPhone", "Canon", "Labubu" и т.п.) определи тип по контексту, то есть по характеристикам товара в этой строке (объем памяти, аккумулятор и т.п.) и добавь товар в максимально близкую по смыслу категорию из списка
